...
def displayUser():
    print()
    print("User Records: \n")
    mycursor.execute("""SELECT UserRecord.UserID,UserRecord.UserName,
                    UserRecord.Password,BookRecord.BookName,
                    BookRecord.BookID
                    FROM UserRecord
...
def searchUser():
    while True:
        ...
        mycursor.execute("SELECT UserID, UserName, Password , BookName, UserRecord.BookID\
                        FROM project.UserRecord LEFT JOIN project.BookRecord\
